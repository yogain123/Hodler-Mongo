...
const { getToken } = require("./configuration/local");
...
router.post("/create/auction", multipartUpload, async function(req, res, next) {
  try {
    ...
    //reqData.image = s3key;
    let randomId = String(new Date().getTime());
    reqData.itemId = randomId;
    reqData._id = randomId;
...
router.get("/fetch/item/:itemId", async function(req, res, next) {
  try {
    ...
    if (item.auctioned) {
      ...
      let highestBidDetails = bidDetails.find(
        item => item.amount == maxBidAmount
      );
  ...
  try {
    ...
    for (let item of allAuctionItem) {
      ...
      if (diff < 0) {
        ...
        if (_.isEmpty(bidDetails)) continue;
        let bidAmount = bidDetails.map(item => +item.amount);
        let maxBidAmount = Math.max(...bidAmount);
        let highestBidDetails = bidDetails.find(
          item => item.amount == maxBidAmount
        ...
        item.highestBidDetails = highestBidDetails;
        let itemDetailFromAuction = await getItemByItemId(
          item.highestBidDetails.itemId
        );
